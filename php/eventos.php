...
if($url_pieces[1]== 'eventos'){
    switch($verb) {
        case 'GET':
                ...
                if(isset($url_pieces[2])) {
                    try {
                        ...
                        $res = $db->query("SELECT * FROM eventos WHERE id_eventos = ?",array($url_pieces[2]));
        ...
        case 'PUT':
                ...
                $id = $url_pieces[2];
                $db->update('eventos', $params);
                header("Location: " . $item['url'], null, 204);
                exit;
                break;
        case 'DELETE':
                $id = $url_pieces[2];
                $ok = $db->delete('eventos', array( 'id_eventos' => $id ) );
                if ($ok) {
                    header("Location: /eventos", null, 204);
                    exit;
                } else {
                    throw new Exception("Evento doesnot exist!", 404);
                };
                break;
        default: 
