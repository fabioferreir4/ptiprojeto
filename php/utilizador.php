    ...
    switch($verb) {
        case 'GET':
                ...
                if(isset($url_pieces[2])) {
                    try {
                        $res = $db->query("SELECT * FROM utilizador WHERE id_user = ?",array($url_pieces[2]));
        ...
        case 'PUT':
                ...
                $id = $url_pieces[2];
                $db->update('utilizador', $params);
                header("Location: " . $item['url'], null, 204);
                exit;
                break;
        case 'DELETE':
                $id = $url_pieces[2];
                $ok = $db->delete('utilizador', array( 'id_user' => $id ) );
                if ($ok) {
                    header("Location: /utilizador", null, 204);
                    exit;
                } else {
                    throw new Exception("Utilizador does not exist!", 404);
                };
                break;
        default: 
