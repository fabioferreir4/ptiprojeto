    ...
    switch($verb) {
        case 'GET':
                ...
                if(isset($url_pieces[2])) {
                    try {
                        $res = $db->query("SELECT * FROM viagem WHERE id_viagem = ?",array($url_pieces[2]));
        ...
        case 'DELETE':
                $id = $url_pieces[2];
                $ok = $db->delete('viagem', array( 'id_viagem' => $id ) );
                if ($ok) {
                    header("Location: /viagem", null, 204);
                    exit;
                } else {
                    throw new Exception("Viagem does not exist!", 404);
                };
                break;
        default: 
